    ...
    if($(e.target).hasClass('voir') && !loadTrig){ // si c'est bien le lien 'voir' et on a pas de chargement en cours
      ...
      $.getJSON(sessionPlusId+'-get',function(data){
        ...
        for(var i=0;i<data.length;i++){ // On ajoute les ligne
          $('<tr id="event-'+data[i]['idevenement']+'"> <td><label class="checkbox inline"><input type="checkbox"/></label></td> <td><a href="#modal-'+data[i]['idevenement']+'" data-toggle="modal">'+data[i]['nomevenement']+'</a></td> <td><a href="#">'+data[i]['nbinscrit']+'</a></td> <td> <div class="modal fade" id="modal-'+data[i]['idevenement']+'"> <div class="modal-header"> <button class="close" data-dismiss="modal">×</button> <h3><span>'+data[i]['nomevenement']+'</span> <i class="icon-pencil"></i></h3> </div> <div class="modal-body"> <ul> <li>Description de l\'évènement : <span>'+data[i]['descevenement']+'</span> <i class="icon-pencil"></i></li> <li>Nombre d\'inscrit : <span>'+data[i]['nbinscrit']+'</span></li> <li>Date du début de l\'évènement : <span class="date">'+data[i]['datedebutevenement']+'</span> <i class="icon-pencil"></i></li> <li>Date de la fin de l\'évènement : <span class="date">'+data[i]['datefinevenement']+'</span> <i class="icon-pencil"></i></li> <li>Emplacement : <span>'+data[i]['emplacementevenement']+'</span> <i class="icon-pencil"></i></li> </ul> </div> <div class="modal-footer"> <a href="event-update" class="btn btn-primary update">Enregistrer les modifications</a><a href="#" class="btn" data-dismiss="modal">Close</a> </div> </div> </td></tr>').appendTo("#events tbody");
        }
